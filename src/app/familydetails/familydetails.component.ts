...
@Component({
  ...
  styleUrls: ['./familydetails.component.sass']
})
...
// (document.getElementById("empFile") as HTMLInputElement).disabled = false;
  this.viewfile=false;
  this.uploadfile=true;
  if(this.employeeForm.get('aadhar')){
    this.uploadfile=false;
  }
  this.employeeForm.get('aadhar')?.valueChanges.subscribe((value) => {
    //alert("please upload adhar");
    if (value) {
      this.uploadfile = true;
      this.viewfile = false;
    } else {
...
onUpdate() {
  ...
  const employeeData = {
    ...
    whatsappNumber: this.employeeForm.get('whatsappNumber')?.value,
    maritalStatus: this.employeeForm.get('maritalStatus')?.value,
    filePath: null // Explicitly setting filePath to null
  };

  // Append employee data as JSON string
  formData.append('data', JSON.stringify(employeeData));

  // If empFile is selected, append it; if not, append null
  if (this.empFile) {
    formData.append('empFile', this.empFile, this.empFile.name);
  } 
  else {
    const emptyFile = new File([], '');  
    formData.append('empFile', emptyFile, '');
  }
